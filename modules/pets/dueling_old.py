...
class PetInDuel:
    ...
    def __init__(self, new_pet: Pet, current_owner: Member):
        ...
        # TODO account for armor penalty
        self.remaining_movement = BASE_MOVEMENT
        self.action_used = False


class JoinData:
    cannot_join: List[int]
    waiting_to_join: List[Member]
    joiners_msg: Message
...
async def start_duel():
    ...
    while current_duel.is_active:
        ...
        for pet_turn in current_duel.pets:
                ...
                try:
                    ...
                    if reply.author.id == pet_turn.owner.id:
                        ...
                        # End of: if response == "move"
                            ...
                            del current_duel.pets[turn_index]
                            break

                        elif response == "end turn":
                            pet_turn.is_turn = False

                        await edit_turn_msg(turn_msg, turn_index)
...
def build_turn_order():
    ...
    current_duel.pets.sort(key=lambda x: x.initiative)


def move_pet(move_to: List, turn_index: int):
    global current_duel

    remaining = current_duel.pets[turn_index].remaining_movement
    moved = abs(current_duel.pets[turn_index].coords[0] - int(move_to[1]) - 1)
    moved += abs(current_duel.pets[turn_index].coords[1] - COLUMN_LETTERS.find(move_to[0]))

    if remaining - moved >= 0:
        current_duel.pets[turn_index].coords[0] = int(move_to[1]) - 1
        current_duel.pets[turn_index].coords[1] = COLUMN_LETTERS.find(move_to[0])

    remaining -= moved


async def edit_turn_msg(msg: Message, turn_index: int):
    global current_duel

    new_msg = f"{nothing_emj}\n{nothing_emj}{nothing_emj}{nothing_emj}{nothing_emj}{nothing_emj}{nothing_emj}" \
              f"**ACTIONS**\n"

    if current_duel.pets[turn_index].remaining_movement == 0 and current_duel.pets[turn_index].action_used:
        new_msg += "~~Attack~~  |  ~~Move~~"

    elif current_duel.pets[turn_index].remaining_movement == 0:
        new_msg += "Attack  |  ~~Move~~"

    elif current_duel.pets[turn_index].action_used:
        new_msg += "~~Attack~~  |  Move"

    else:
        new_msg += "Attack  |  Move"

    new_msg += "  |  Skill  |  Item  |  End Turn  |  Surrender"
...
async def duel(ctx: Context):
    ...
    if not len(command_msg) == 2:
        await ctx.send(f"{host.mention} Invalid !duel command. It should be !duel X (Where X is the diggities you "
                       f"would like to wager on this duel.")
        await ctx.message.delete()
        return

    # In new duel: Store context, set duel as active, and set wager
    current_duel.duel_context = ctx
    current_duel.is_active = True
    current_duel.waiting_state = ResponseStates.none.value
    current_duel.wager = int(command_msg[1].strip())

    # Delete calling message
    await ctx.message.delete()

    # Store host's pet as a dueling pet
    current_duel.pets = [PetInDuel(db_pets.get_pet(int(host_id)), host)]

    # Use singular form of digities for 1 in wager
    diggities = "diggity" if current_duel.wager == 1 else "diggities"

    # Output new active duel message and store the contents
    duel_join_msg = await ctx.send(f"{host.mention} has opened a duel for **{current_duel.wager} {diggities}**!\n"
    ...
    duel_joined = False
    current_duel.waiting_state = ResponseStates.join.value
    current_duel.join_data.cannot_join = [int(host_id)]
    ...
    while len(current_duel.pets) < 4:
        ...
        # New duel timed out with no new joiners
            ...
            if not duel_joined:
                no_one_dueling_msg = await ctx.send(f"{host.mention} Seems like no one was willing to duel. "
                                                    f"Try again later.")
                return
            else:
            ...
            if reply.content.lower() == "join" and int(reply.author.id) not in current_duel.join_data.cannot_join:

                # Checks that the joiner has a pet
                if not db_pets.check_pet(int(reply.author.id)):
                    ...
                    # Join request timed out without host response
                        ...
                        if host_reply.author.id == ctx.author.id:
                            ...
                            if host_reply.content.lower() == "accept":
                                host_responded = True
                                current_duel.join_data.cannot_join.append(reply.author.id)
                                joiners_pet = db_pets.get_pet(int(reply.author.id))
            ...
            elif duel_joined and reply.content.lower() == "start" and reply.author.id == host_id:
                await duel_join_msg.edit(content=f"Duel is about to begin {loading_emj}")
                break
    ...
    if len(current_duel.pets) > 2:
        ...
        current_duel.type = 0
        duel_type_msg = await ctx.send(f"{host.mention}\n"
        ...
        while current_duel.type == 0:
            ...
            # Mode select timed out and duel is cancelled
                ...
                type_not_selected_msg = await ctx.send(f"{host.mention} You didn't select a duel type in time. "
                                                       f"Duel cancelled.")
                return

            else:
                # Only host may choose mode
                if reply.author.id == host_id:
    ...
    if current_duel.type == DuelType.team.value:
        team_select_text = f"{host.mention}, enter the number of user you want to team with or send \"random\"\n"
        ...
        while host_teammate == -1:
            ...
            # Teammate select timed out and a random teammate was assigned
                ...
                type_not_selected_msg = await ctx.send(f"{host.mention} Times Up!")
        ...
        if not host_teammate == 1:
            current_duel.pets[1], current_duel.pets[host_teammate] = \
                current_duel.pets[host_teammate], current_duel.pets[1]
...
async def join(ctx: Context):
    ...
    joiner_id = int(ctx.author.id)
    if joiner_id in current_duel.join_data.cannot_join:
        return
